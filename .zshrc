...
export ZSH="/Users/paulwendt/.oh-my-zsh"
...
# Commands I need for work
export NVM_DIR=~/.nvm
source $(brew --prefix nvm)/nvm.sh

# Commands I want
alias activate='source .venv/bin/activate'
alias ri='nc -l localhost 1234'
alias wi='nc localhost 1234'
alias mark='m=$(pwd)'
...
precmd() {
    # for Version Control System stuff
    vcs_info;

    # For AWS profiling
    if [[ $(cat ~/.aws/credentials | grep default) ]]; then
        COLOR=$(get_color)
        ACCOUNT="unknown"
        export ACCT="%{$fg[$COLOR]%}($ACCOUNT)%{$reset_color%}"
    else
        export ACCT=""
    fi;
}
...
setopt PROMPT_SUBST
PROMPT='[%n@%m %1~]%F{green}(${vcs_info_msg_0_})%F{white}${ACCT}$ '
[ -f ~/.fzf.zsh ] && source ~/.fzf.zsh

# Git stuff
# rename_branch is symlinked to /Users/paulwendt/utils/bash/rename_branch.sh
# push_branch exists in /usr/local/bin. Not sure why I made it there
